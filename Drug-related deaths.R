...
# Data file for 2002-2005 - Both sexes
...
# Create a key variable in SMR extract (rdno_entry_no_yr) so can join to NRS DRD file and a 5 yr age group variable (age_grp).
...
drug_deaths_smr <- drug_deaths_smr |> 
  ...
  mutate(entry_no = case_when(nchar(entry_no) == 2 ~ paste0("0", entry_no),
         ...
         # Create rdno_entry_no_yr key variable by concatenating relevant existing variables.
...
# Remove from workspace data tables no longer required
...
# drug_deaths_ca_male <- readRDS(paste0(profiles_data_folder, "/Prepared Data/drug_deaths_male_raw.rds"))
# drug_deaths_ca_female <- readRDS(paste0(profiles_data_folder, "/Prepared Data/drug_deaths_female_raw.rds"))
...
# Update both sexes output data frame from main_analysis() function with DRD data for 2002 to 2005 as drugs team only provide NRS data from 2006 
# onwards and sort by year
drug_deaths_bothsexes <- rbind(main_analysis_result, drug_deaths_02_05) |>
  arrange(year)

# Delete partial DRD file automatically saved from main_analysis() function as doesn't include full time series (missing 2002-2005) as we
# don't want this loaded into shiny tool
file.remove(file.path(profiles_data_folder, "Data to be checked", "drug_deaths_shiny.rds"))
file.remove(file.path(profiles_data_folder, "Data to be checked", "drug_deaths_shiny.csv"))

# Save final output files for DRDs for both sexes
...
# Run QA report on final output file (both sexes) for shiny app
...
# Update male output data frame from main_analysis() function with DRD data for 2002-2006 to 2005-2009 (5-year aggregates) as drugs team only 
# provide NRS data from 2006-2010 onwards and sort by year
drug_deaths_male <- rbind(main_analysis_result, drug_deaths_male_02_05)|>
  arrange(year)

# Delete partial DRD file automatically saved from main_analysis() function as doesn't include full time series (missing 2002-2006 to 2005-2009) 
# as we don't want this loaded into shiny tool
file.remove(file.path(profiles_data_folder, "Data to be checked", "drug_deaths_male_shiny.rds"))
file.remove(file.path(profiles_data_folder, "Data to be checked", "drug_deaths_male_shiny.csv"))

# Save final output files for DRDs for males
...
# Run QA report on final output file (males) for shiny app
...
# Update female output data frame from main_analysis() function with DRD data for 2002-2006 to 2005-2009 (5-year aggregates) as drugs team only
# provide NRS data from 2006-2010 onwards and sort by year
...
file.remove(file.path(profiles_data_folder, "Data to be checked", "drug_deaths_female_shiny.csv"))

# Save final output files for females

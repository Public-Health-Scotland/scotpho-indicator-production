...
# 1. Inter-agency referral discussion (IRD) (ind_id = 30169)
# 2. Children on the child protection register (ind_id = 13035)
# 3. Child protection with parental drug misuse (ind_id = 4130)
# 4. Child protection with parental alcohol misuse (ind_id = 4110)
# 5. Child protection with parental drug or alcohol misuse (ind_id = 4153)

# Full definition: (indicator 1) (ONLY ONE DATA POINT AT PRESENT)
...
# Full definition: (indicators 3-5)
# Rate of Child Protection Case Conferences (now Child Protection Planning Meetings) where parental drug, alcohol or substance misuse (ie drug &/or alcohol)
...
# This script analyses Scottish Government data from a variety of spreadsheets in differing formats
...
# POP LOOKUP : Make pop lookup to use for popgroups data:
# Make a pop lookup (0-17y, with age groups and sex splits for Scotland)
...
# version for LA whole populations:
pop_la <- popfile %>%
  mutate(split_name = "Total", 
         split_value = "Total") %>%
  select(-agegp, -sex)
# version for Scotland whole population:
pop_scot <- popfile %>%
  mutate(code = "S00000001",
         split_name = "Total", 
         split_value = "Total") %>%
  select(-agegp, -sex)
# version for Scotland population by sex:
...
# version for Scotland population by age group:
...
# Combine all versions and aggregate by groups
...
#remove pop files no longer needed
rm(pop_la,pop_scot, pop_scot_agegp, pop_scot_sex,pop_scot_agegp_total,pop_scot_sex_total,popfile)
...
# Function to fix all the non-standard area names found in theses data and add the codes from the geo_lookup
add_area_codes <- function(df) {
  
  df <- df %>%
    mutate(
      areatype = "Council area",
      areaname = str_replace(areaname, " and "," & "),
      areaname = case_when(str_detect(areaname, "Shetland") ~ "Shetland Islands", TRUE ~ areaname),
      areaname = case_when(str_detect(areaname, "Glasgow") ~ "Glasgow City", TRUE ~ areaname),
      areaname = case_when(str_detect(areaname, "Eilean") ~ "Na h-Eileanan Siar", TRUE ~ areaname),
      areaname = case_when(str_detect(areaname, "Edinburgh") ~ "City of Edinburgh", TRUE ~ areaname),
      areaname = case_when(str_detect(areaname, "Orkney") ~ "Orkney Islands", TRUE ~ areaname)
    ) %>%
...
########################################################################################.
# 1. INTER-AGENCY REFERRAL DISCUSSION (IRD) DATA ----
########################################################################################.
...
saveRDS(IRD_2023, file=paste0(profiles_data_folder, '/Prepared Data/IRD_2023_raw.rds'))

# Run main analysis function to aggregate and calculate rates (CA to higher geogs)
# safe to use regular CA under 18 pop lookup as no age breakdowns 
...
register_scot_agegp <- rbind(register_2000to2012, register_2013to2024) %>%
  ...
  select(-sex)
# Combine to give popgroups data, adding required columns and denominators, and calculating rates
register_scot <- rbind(register_scot_agegp, register_scot_sex) %>%
  mutate(code = "S00000001",
         ind_id = 13035,
         trend_axis = as.character(year),
         def_period = paste0(year, " - snapshot"),
         split_value = factor(split_value,
                              labels = c("Male", "Female", "All", "Unborn", "0-4", "5-10", "11-15", "16-17", "Total"),
                              levels = c("Male", "Female", "All", "Unborn", "0-4", "5-10", "11-15", "16-17", "Total"))) %>%
  merge(y=pop_lookup, by=c("year", "code", "split_name", "split_value")) %>% # unborn agegp dropped in this stage as no denominators
...
register_la_2013to2024 <- import_wide_data(filename = publication_2023, sheetnum = "1.2", range = "A40:Y72", non_num_cols = c(1)) %>%
  ...
  add_area_codes(.) %>%
  select(code, year, numerator = Number)
...
########################################################################################.
# 3. CHILD PROTECTION CASES WITH CONCERNS RECORDED AS ALCOHOL/DRUGS/SUBSTANCE ABUSE ----
########################################################################################.

# Import the data (only main data file is possible, no popgroups)

...
#Read in old indicator data file from network /Shiny data folder - if one exists
old_indicator_filepath <- paste0(data_folder, "Shiny Data/", filename, "_shiny.csv") #create value which is file path to old file
...
if(file.exists(old_indicator_filepath)) {
  skip_historic_file_checks <-"n"
  old_data_indicator <- read_csv(paste0(data_folder, "Shiny Data/", filename, "_shiny.csv")) #read in file if one exists
} else {
  skip_historic_file_checks <-"y"
  old_indicator_filepath <-"no historic file available for comparison" #if no historic file exists set value to no file available
}

#Logical test of which optional geographies are present in data set
# values will return TRUE/FALSE according to whether each geogrpahy type is present
...
Conditional formatting shoudl highlight where geographies present are not equal to expected (e.g. 14 NHS boards, 32 councils or 31 HSCP)<br>
...
The table below compares the numerator, measure and CI of the latest file against the last shiny data file prepared (if historic file available).<br>
...
#do nothing if checks to be skipped
}
```
```{r, out.width = "400px", echo=FALSE}
...
ft_check3%>%
  ...
  paste0("No historic data file present to carry out comparisons")
}

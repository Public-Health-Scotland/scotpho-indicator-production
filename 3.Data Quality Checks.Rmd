...
<br>
#### Sometimes mistakes creep in when aggregating larger geographies from smaller ones. The following checks look at whether rates (& numerators if available)
#### seem to make sense. Council areas can be aggregated and the totals should be equal to health boards (eg Borders council is the same as NHS borders, 5 councils make up #### NGS GGC) and HSCP partnerships are generally the same as HSCPs. Numerators for HSCP locality should alos generally sum up to the HSCP partnership total.
...
Checking if the numerators for councils sum to health board (bear in mind some indicators there are don't have numerators so this check won't run) 
...
```
If dataset contains numerators - check that the sum of council area figures matches the health board totals .<br>
```{r, echo=FALSE}

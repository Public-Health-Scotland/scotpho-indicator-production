...
hscp_quintiles <- any(substr(data_indicator$code,1,3) == "S37")
pd_quintiles <- any(substr(data_indicator$code,1,3) == "S32")
...
Health and Social Care Partnership: `r hscp_quintiles` <br>
Police division: `r pd_quintiles` <br>
...
# step above creates a column for each quint type
# need to create cols for quintiles not found in dataset
# and populate with NA so that the checks below work properly
if(hscp_quintiles == FALSE){
  geo_checks$S37 <- NA
}

if(pd_quintiles == FALSE){
  geo_checks$S32 <- NA
}

if(ca_quintiles == FALSE){
  geo_checks$S12 <- NA
}

if(hb_quintiles == FALSE){
  geo_checks$S08 <- NA
}

if(scot_quintiles == FALSE){
  geo_checks$S00 <- NA
}
...
if(ca_quintiles == TRUE) {ft_sc_quin_check<- ft_sc_quin_check %>%  color(~ S12!=32, ~S12, color = "red")} # Should be 32 council areas
if(hscp_quintiles == TRUE) {ft_sc_quin_check<- ft_sc_quin_check %>%  color(~ S37!=31, ~S37, color = "red")} # Should be 31 HSCP
if(pd_quintiles == TRUE) {ft_sc_quin_check<- ft_sc_quin_check %>%  color(~ S32!=13, ~S32, color = "red")} # Should be 13 police divisions
...
ft_hb_quin_check <- geo_checks %>%
  ...
  color(~ !is.na(S37), ~S37, color = "red") %>% # Should be no records for HSCPs
  color(~ !is.na(S32), ~S32, color = "red") %>% # Should be no records for PDs
  autofit()}
...
if(hscp_quintiles == TRUE){
# flextable on hscp quintiles - summary table of years and geography types present
ft_hscp_quin_check <- geo_checks %>%
  subset(quint_type=="hscp_quin") %>%
  flextable() %>%
  color(~ !is.na(S00), ~S00, color = "red") %>% # Should be no records for scotland code
  color(~ !is.na(S12), ~S12, color = "red") %>% # Should be no records for council codes
  color(~ !is.na(S08), ~S08, color = "red") %>% # Should be no records for HB codes
  color(~ S37!=31, ~S37, color = "red") %>% # Should be 31 records for HSCP quintiles
  color(~ !is.na(S32), ~S32, color = "red") %>% # Should be no records for PDs
autofit()
}
...
paste0("No health board quintiles present")
}   

if(ca_quintiles==TRUE){
ft_ca_quin_check #print flextable
} else {
paste0("No within council quintiles present")
} 

if(hscp_quintiles==TRUE){
ft_hscp_quin_check #print flextable
} else {
paste0("No within HSCP quintiles present")
} 
...
prep_areatype_quintile_check <- function (areatype) {
  ...
  } else if (areatype=="HSCP") {
    qtype <- "hscp_quin"
    warning <- hscp_quintile_warning
  }
...
# Quintile checks if hscp_quin quintiles are in the data:
if (hscp_quintiles) {
  # if hscp quintile sum not equal to 6 there's an issue (since within geography deprivation should always have 5 quintiles plus a total)
  hscp_quintile_warning <- quintile_checks %>%
    subset(quint_type == "hscp_quin") %>%
    mutate(q_count = ifelse(count_q!=6, TRUE, FALSE)) %>%
    summarise(quint_error_total=sum(q_count,na.rm=TRUE))
  #Text output when hscp quintiles not correct
  ifelse(hscp_quintile_warning$quint_error_total == 0, "There are no gaps in the HSCP-level quintile data. :-)",
  "STOP AND CHECK....looks like not all geographies have correct number of HSCP-level quintiles.")
prep_areatype_quintile_check("HSCP")
}
# Message if no ca/hb/hscp quintiles are in the data:
if(hb_quintiles==FALSE & ca_quintiles==FALSE & hscp_quintiles==FALSE) {
  "There are no HB- or CA- or HSCP-level quintiles in the data."
}
...
# check if the data has numerators:
numerators_present <-ifelse(all(data_indicator$numerator == "", na.rm = TRUE)|
                              all(is.na(data_indicator$numerator)),FALSE,TRUE)

# check for small counts if numerators present
if (numerators_present==TRUE) {
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    select(code,code_name, year, sex, numerator, denominator, rate, quint_type, quintile, geo_type, par, rii, sii,trend_axis) %>%
    ...
    #Create trend plot
    rate_trend <- plot_ly(data= simd_trend_data(), x=~trend_axis,hoverinfo="text") %>%

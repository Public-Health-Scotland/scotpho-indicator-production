...
# path to ScotPHO data folder - within this folder there are different sub-folders
...
deprivation_analysis <- function(filename, yearstart, yearend, time_agg, 
  ...
  # (ADP, HSCP AND PD ARE BIGGER THAN CA, SO COULD THEY BE INCLUDED HERE?)
  ...
  # that therefore couldn't be assigned to any SIMD quintiles/deciles
   ...
   if(measure %in% c("stdrate", "crude")){
     ...
     population_lookup <- readRDS(file.path(population_lookups, "basefile_deprivation.rds")) |> #file with PD, ADP, HSCP in it (no DZ)

...
```{r, include=FALSE}
#chunk can be removed when rendered in funtion

#checks to build
# 1- expected geographies present
# 2- expected quintiles present
# 3- compare to previous file
# 4- rates within ci - will all indicators have ci?
# 5 - small numbers - do all indicators have numerators?
# 6 - visualise rates by simd (shiny)
# 7 - visualise sii and rii (shiny)
# 8 - visualise par (shiny)
...
#filename="alcohol_stays_depr"
#filename="healthy_birth_depr"
#filename="test_early_cancer_deaths_depr"
...
#Test which optional geographies are present in dataset - not sure this code is needed?
...
if (qa==TRUE){ #if qa is true then open old indicator file for comparisons
...
**Old** indicator file (for comparisons):  /`r if(qa==TRUE){(paste0(data_folder, "Shiny Data/", old_filename, ".rds"))}`
...
Conditional formatting will show up if gdistinct geography counts are not as expected 
```{r,}
#print flextable
ft_hb_quin_check
ft_ca_quin_check
...
##Data check 5:
#### Small numbers
Look at proportion of areas and quintiles where counts are <5 and estimates may not be robust. 
...
linear_trends <- data_indicator  %>%
  ...
  spread(pattern,count) %>%
  mutate(percent_linear=100-round(non_linear/sum*100,1))

ft <- linear_trends %>%
 select(geo_type,quint_type,year,ascending:non_linear,sum,percent_linear) %>%
  flextable() %>%
...
shinyApp(
  ...
  ui = fluidPage(
     fluidRow(
     column(3,selectInput("code_selected", "Area:", choices = unique(data_indicator$code),multiple = FALSE, selected="S00000001")),
  ...
  server = function(input, output) {
    ...
    select(code, year, numerator, denominator, rate, quint_type, quintile, geo_type, par, rii, sii) %>%
    subset(quint_type==input$quint_selected & code==input$code_selected) %>%
...
# quint_type_selection <- "sc_quin"
# year_selection <- 2015
# geo_selection <- "S00000001"
  

# RATE-BAR CHART

# # reactive dataset for bar chart
# rate_bar_data <- data_indicator %>%
# select(code, year, numerator, denominator, rate, quint_type, quintile, geo_type, par) %>%
#   subset(quint_type==quint_type_selection & year==year_selection & code==geo_selection) %>%
#   mutate(average=rate[quintile=="Total"]) %>%
#     filter(quintile !="Total") %>%
#     droplevels()
# 
#     #Palette for plot 
#     pal_rate_bar <- case_when(rate_bar_data$quintile == "1" ~ '#022031', 
#                               rate_bar_data$quintile == "2" ~ '#313695', 
#                               rate_bar_data$quintile == "3" ~ '#4575b4', 
#                               rate_bar_data$quintile == "4" ~ '#74add1', 
#                               rate_bar_data$quintile == "5" ~ '#abd9e9')
# 
# 
#  plot_ly(data=rate_bar_data, x=~quintile) %>%
#       #Comparator line
#      add_trace(y = ~average, name = "Average", type = 'scatter', mode = 'lines',
#               line = list(color = '#FF0000'), hoverinfo="skip") %>%
#   #quintile bars   
...
# RATE-TREND CHART
# 
# # reactive dataset for trend chart
# rate_trend_data <- data_indicator %>%
#   select(code, year, numerator, denominator, rate, quint_type, quintile, geo_type, sii, rii, par) %>%
#   subset(quint_type==quint_type_selection & code==geo_selection) %>%
#     arrange(quintile) %>% #this is needed to make palette assignments work well
#     droplevels()
# 
#         #Palette for plot 
#     pal_simd_trend <- case_when(rate_trend_data$quintile == "1" ~ '#022031', 
#                                 rate_trend_data$quintile == "2" ~ '#313695', 
#                                 rate_trend_data$quintile == "3" ~ '#4575b4', 
#                                 rate_trend_data$quintile == "4" ~ '#74add1', 
#                                 rate_trend_data$quintile == "5" ~ '#abd9e9',
#                                 rate_trend_data$quintile == "Total" ~ '#FF0000')
# 
#  #Creating plot
#     plot_ly(data=rate_trend_data, x=~year, 
#                                 hoverinfo="text") %>%
...
# 
# 
# index_data <- rate_trend_data  %>%  filter((quintile == "Total"))
# 
# plot_ly(data=index_data, x=~year,
#                          hoverinfo="text") %>%
...
#     rate_parbar_data <- rate_bar_data %>%
#       mutate(baseline = rate[quintile == "5"],
#              diff_baseline = rate - rate[quintile == "5"]) %>% 
#       droplevels()
# 
# 
# plot_ly(data = rate_parbar_data, x = ~quintile,hoverinfo="text") %>%
#       add_bars(y = ~baseline, name= "", marker = list(color = "#4da6ff"), showlegend = FALSE) %>%   
#       add_bars(y = ~diff_baseline, name = "Attributable to deprivation", 
#                marker = list(color = "#ffa64d"), showlegend = FALSE) %>% 
#       layout(title=year_selection,
#         bargap = 0.1, barmode = 'stack', showlegend = T, 
#              legend = list(x = 0.9, y = 0.9),
#              margin = list(b = 140)) %>%
#       config(displayModeBar = FALSE, displaylogo = F, editable =F) # taking out toolbar

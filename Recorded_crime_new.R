...
rec_crime_2020 <- read_excel(paste0(filepath, "recorded-2020.xlsx"), sheet = 2) |>
...
crime_janmar_20 <- rec_crime_2020 |> 
  filter(rec_date <= '2020-03-31')

#Read in current fy data from previous calendar year (Apr-Dec)
crime_aprdec_19 <- readRDS(paste0(filepath, 'recorded_crime_next_fy_DO_NOT_DELETE.rds'))

#Combine both calendar years to get current financial year
crime_19_20 <- rbind(crime_janmar_20, crime_aprdec_19)

#Tidy up current financial year
crime_19_20 <- crime_19_20 |> 
...
recorded_crime <- rbind(crime_historic, crime_19_20)
...
crime_aprdec_20 <- rec_crime_2020 |> 
  filter(rec_date > '2020-03-31')


saveRDS(crime_aprdec_20, file=paste0(filepath, 'recorded_crime_next_fy_DO_NOT_DELETE.rds'))

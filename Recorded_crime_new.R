...
rec_crime_2019 <- read_excel(paste0(filepath, "recorded-2019.xlsx"), sheet = 2) |>
...
crime_janmar_19 <- rec_crime_2019 |> 
  filter(rec_date <= '2019-03-31')

#Read in current fy data from previous calendar year (Apr-Dec)
crime_aprdec_18 <- readRDS(paste0(filepath, 'recorded_crime_next_fy_DO_NOT_DELETE.rds'))

#Combine both calendar years to get current financial year
crime_18_19 <- rbind(crime_janmar_19, crime_aprdec_18)

#Tidy up current financial year
crime_18_19 <- crime_18_19 |> 
...
recorded_crime <- rbind(crime_historic, crime_18_19)
...
crime_aprdec_19 <- rec_crime_2019 |> 
  filter(rec_date > '2019-03-31')


saveRDS(crime_aprdec_19, file=paste0(filepath, 'recorded_crime_next_fy_DO_NOT_DELETE.rds'))

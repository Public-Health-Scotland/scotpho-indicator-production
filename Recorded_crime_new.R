...
rec_crime_2015 <- read_excel(paste0(filepath, "recorded-2015.xlsx"), sheet = 2) |>
...
crime_janmar_15 <- rec_crime_2015 |> 
  filter(rec_date <= '2015-03-31')

#Read in current fy data from previous calendar year (Apr-Dec)
crime_aprdec_14 <- readRDS(paste0(filepath, 'recorded_crime_next_fy_DO_NOT_DELETE.rds'))

#Combine both calendar years to get current financial year
crime_14_15 <- rbind(crime_janmar_15, crime_aprdec_14)

#Tidy up current financial year
crime_14_15 <- crime_14_15 |> 
...
recorded_crime <- rbind(crime_historic, crime_14_15)
...
crime_aprdec_15 <- rec_crime_2015 |> 
  filter(rec_date > '2015-03-31')


saveRDS(crime_aprdec_15, file=paste0(filepath, 'recorded_crime_next_fy_DO_NOT_DELETE.rds'))

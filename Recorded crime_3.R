...
crime_dz_code <- left_join(rec_crime_filtered, lookup, by = c("datazone", "division_name")) |>
  group_by(year, IZ2011_Code) |> #aggregate datazones into intermediate zones and sum numerators
  summarise(numerator = sum(numerator)) |> 
  rename(intzone11 = IZ2011_Code) |> #change name for analysis function
  select(intzone11, everything()) |> #move IZ to first col
  ungroup()


#Remove problem intermediate zones
crime_imz <- crime_dz_code |> 
  mutate(intzone11 = case_when(intzone11 %in% c("S02001528","S02001953","S02002233","S02001475") ~ "NULL",
                   TRUE ~ intzone11))


#Save prepared data for analysis functions
saveRDS(crime_imz, file=paste0(profiles_data_folder, '/Prepared Data/recorded_crime_raw.rds'))
saveRDS(crime_imz, file=paste0(profiles_data_folder, '/Prepared Data/recorded_crime_depr_raw.rds'))


###############################################.
## Part 2 - Run analysis functions ----
###############################################.
main_analysis(filename = "recorded_crime", geography = "intzone11", measure = "crude",

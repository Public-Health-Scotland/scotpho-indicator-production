...
# Prepare for IR - dz01 & dz11
sadIR_data_raw <- sad_data_raw %>%
  filter(dz != "dz01" | year != "2014")

sadIR_data_raw <- select(sadIR_data_raw,-c(ca, dz))

saveRDS(sadIR_data_raw, file=paste0(data_folder, 'Prepared Data/DZ_single_dwellings_IR_raw.rds'))

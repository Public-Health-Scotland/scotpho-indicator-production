...
# 1a - Extract data ----
#########################.
#Note for 2026 update - try to use opendatascot package. Couldn't do historically due to file size or VPN issues
...
dz11_sad <- dz11_sad |>
  ...
  filter(!(year == 2014 & dz_number <= 1006505)) |> 
  select(-dz_number)
...
sad_data_depr <- sad_data_extract |> 
  filter(str_detect(datazone, "S01") & year < 2024) |>  #keep datazones only excluding 2024 data
  mutate(dz_number = str_sub(datazone, start = 2)) |> #get each dz number without the S-
  mutate(dz_number = as.numeric(dz_number)) |> 
  filter(!(year == 2014 & dz_number <= 1006505)) |>  #exclude the 2001 dzs for 2014 leaving only 2011
  select(-dz_number)

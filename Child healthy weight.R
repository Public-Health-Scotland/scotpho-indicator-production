...
child_weight11 <- read.spss( paste0(data_folder, "Received Data/IR2018-00101_DZ2011 Child weight.sav"), 
...
###############################################.
# Datazone2001 
child_weight01 <- read.spss( paste0(data_folder, "Received Data/IR2018-00101_DZ2001 Child weight.sav"), 
                             to.data.frame=TRUE, use.value.labels=FALSE)%>% 
  setNames(tolower(names(.))) %>%  #variable names to lower case
...
analyze_first(filename = "child_healthyweight", geography = "datazone11", 
              measure = "percent", yearstart = 2002, yearend = 2016, time_agg = 1)

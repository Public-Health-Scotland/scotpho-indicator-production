...
# HLE can only be generated at Scotland, NHS board and CA level.  It is NOT possibly to generate at smaller geographies
...
###############################################.
# Healthy life expectancy data by CA & NHS board
###############################################.
...
simd <- c("all")
urban_rural <- c("all")
age_select <- c("0-years") #include filter in opendata extraction as it is needed to ensure number of rows to extract is within limits however (it returns '0 years' and '90 years')
...
hle = ods_dataset("healthy-life-expectancy", refPeriod = date_range_hle,
  ...
  filter(age == age_select) %>%  # filter again as this will select correctly filter only '0 years' (ie healthy life expectancy at birth)
...
hle <- hle %>%
  ...
  mutate(numerator=" ", #insert column where numerator would ordinarily be - there is no numerator for LE
         ind_id= case_when(sex=="1" ~ "20103", #male indicator number
                           sex=="2" ~ "20104", #female indicator number
                           TRUE ~"x"),

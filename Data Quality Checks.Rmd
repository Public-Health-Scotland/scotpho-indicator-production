...
iz <- ifelse(as.numeric(count(data_indicator %>% filter(substr(code,1,3) == "S02"))) > 0,TRUE, FALSE)
adp <- ifelse(as.numeric(count(data_indicator %>% filter(substr(code,1,3) == "S11"))) > 0,TRUE, FALSE)
locality <- ifelse(as.numeric(count(data_indicator %>% filter(substr(code,1,3) == "S99"))) > 0,TRUE, FALSE)
...
check_codes <- c("S00000001","S08000015","S12000010",check_extras) 

##If IZ true add IZ geography to check codes
if(iz==TRUE){
  check_codes <- c(check_codes,"S02001236")
}

##If ADP true add ADP geography to check codes
if(adp==TRUE){
  check_codes <- c(check_codes,"S11000006")
}
...
edinburgh_data <- data_indicator %>%        # create data frame filtered for edinburgh geographies
  ...
  mutate(ca_partnership=round(S12-S37,2),  #create fields that test for differences - use rounded figures
...
if(locality==TRUE && all(data_indicator$numerator != "") && sum(edinburgh_data$check_tot) !=0){  #if numerator column not blank and sum of check column is not zero create a flextable showing where differences occur
...
# Print statement summarising
case_when((all(data_indicator$numerator != "") && sum(edinburgh_data$check_tot)==0) ~ 
              "Numerators for Edinburgh council, HSCP and (if present) localities all agree :-)",
            (all(data_indicator$numerator != "") && sum(edinburgh_data$check_tot)!=0) ~
              "uh-oh. Doesn't look like the numerators for Edinburgh localities, hscp &/or council areas tally",
             TRUE ~ "No numerator data")  

# If there is any disagreement between numerators then print flextable detailing differences
...
The table below shows area types where more than 10% of the rows contain numerators <5.
...
**note** this table doesn't seem to have issues printing in rmarkdown if it is very long.  If you find the table is truncated and the report does not complete too many rows with counts <5 maybe be the cause.
...
###Yay!
<br>
**Don't forget to update Googledocs planning sheet once checks are complete**

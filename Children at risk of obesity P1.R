...
# 15002 - Children at risk of obesity in Primary 1
# note this can be updated a the same time as Child Healthy Weight in P1 indicator as uses same dataset (see Child Healhy Weight.R script)
...
# data based on 2011 datazones - for main and deprivation analysis (to match post 2013 data to later versions of SIMD that are DZ11 based)
# data based on 2001 datazones - for deprivation analysis only (to match pre-2013 data to early versions of SIMD that are DZ01 based)
...
# create temp data
# calculating the numerator (total at risk of obesity using epidemiological category (BMI>=95th))
# and denominator (total reviewed) for each datazone and year
main_temp <- dz_11 |>
  group_by(datazone, year) |>
  summarise(numerator = sum(Obese),
            denominator = sum(tot), 
            .groups = "drop")

# save temp file to pass to analysis function
saveRDS(main_temp, file.path(profiles_data_folder, "Prepared Data", "15002_child_obesity_risk_raw.rds"))
...
# smaller than the denominator e.g more P1 children reviewed in an area than 
...
main_analysis(filename = "15002_child_obesity_risk", geography = "datazone11", ind_id = 15002,
...
saveRDS(popgroups_temp$M, file.path(profiles_data_folder, "Prepared Data", "15002_child_obesity_risk_males_raw.rds"))
saveRDS(popgroups_temp$`F`, file.path(profiles_data_folder, "Prepared Data", "15002_child_obesity_risk_females_raw.rds"))
...
main_analysis(filename = "15002_child_obesity_risk_males", geography = "datazone11", ind_id = 15002,
              yearstart = 2002, yearend = 2024, time_agg = 1, year_type = "school",
              measure = "perc_pcf", pop="DZ11_pop_5_male", QA = FALSE)

# calculate rates for females
main_analysis(filename = "15002_child_obesity_risk_females", geography = "datazone11", ind_id = 15002,
              yearstart = 2002, yearend = 2024, time_agg = 1, year_type = "school",
              measure = "perc_pcf", pop="DZ11_pop_5_female", QA = FALSE)

# read males results back in 
males <- readRDS(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_males_shiny.rds")) |>
  mutate(split_value = "Males")

# read females results back in 
females <- readRDS(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_females_shiny.rds")) |>
  mutate(split_value = "Females")

# read totals results back in (i.e. result from main analysis)
all <- readRDS(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_shiny.rds")) |>
...
# save final file 
saveRDS(popgroups_data, file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_shiny_popgrp.rds"))
write.csv(popgroups_data, file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_shiny_popgrp.csv"), row.names = FALSE)
...
file.remove(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_females_shiny.rds"))
file.remove(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_females_shiny.csv"))
file.remove(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_males_shiny.rds"))
file.remove(file.path(profiles_data_folder, "Data to be checked", "15002_child_obesity_risk_males_shiny.csv"))
...
saveRDS(depr_temp, file.path(profiles_data_folder, "Prepared Data", "15002_child_obesity_risk_depr_raw.rds"))


# create final deprivation file
deprivation_analysis(filename = "15002_child_obesity_risk_depr", ind_id = 15002,
...
suppress_data <- function(file = c("main", "deprivation", "popgrp")){
  ...
  filename <- switch(file,
                     "main" = "15002_child_obesity_risk_shiny",
                     "deprivation" = "15002_child_obesity_risk_depr_ineq",
                     "popgrp" = "15002_child_obesity_risk_shiny_popgrp"
  )

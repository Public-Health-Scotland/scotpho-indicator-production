...
## This script analyses SG education data on the School leaver with 1 or more qualification at SCQF Level 6
...
################################################################################
# read in csv
school_leaver <- read.csv(paste0(data_folder, "Received Data/school_leaver_SCQFL6_raw.csv")) %>% 
  as_tibble()
...
################################################################################
# read in data
school_leaver_destinations <- read_rds(paste0(data_folder, "Prepared Data/school_leaver_SCQFL6_raw.rds"))

saveRDS(school_leaver_destinations, file=paste0(data_folder, "Prepared Data/school_leaver_SCQFL6_raw.rds"))


###############################################.
## Part 2 - Run analysis functions ----
###############################################.
analyze_first(filename = "school_leaver_SCQFL6", geography = "council", 
              measure = "percent", yearstart = 2009, yearend = 2019, 
              time_agg = 1)


# then complete analysis with the updated '_formatted.rds' file
analyze_second(filename = "school_leaver_SCQFL6", measure = "percent", 
...
saveRDS(final_result, file = paste0(data_folder, "Data to be checked/school_leaver_SCQFL6_shiny.rds"))
write_csv(final_result, path = paste0(data_folder, "Data to be checked/school_leaver_SCQFL6_shiny.csv"))
